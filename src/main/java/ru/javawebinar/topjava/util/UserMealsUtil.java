...
//        map.keySet().stream().forEach(ldt -> System.out.println(ldt +" : " + map.get(ldt)));
        return mealList.stream().filter(um -> TimeUtil.isBetween(um.getDateTime().toLocalTime(),startTime,endTime))
                .map(um -> makeWithExceed(um, map.get(um.getDateTime().toLocalDate()) > caloriesPerDay))
                .collect(Collectors.toList());

    }
    public static UserMealWithExceed makeWithExceed(UserMeal userMeal,boolean exceed){
